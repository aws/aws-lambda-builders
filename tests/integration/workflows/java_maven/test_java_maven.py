...
class TestJavaMaven(TestIntegBase):
    SINGLE_BUILD_TEST_DATA_DIR = os.path.join(os.path.dirname(__file__), "testdata", "single-build")

    def setUp(self):
        super(TestJavaMaven, self).setUp()
        self.artifacts_dir = tempfile.mkdtemp()
        self.scratch_dir = tempfile.mkdtemp()
        self.ro_source(self.SINGLE_BUILD_TEST_DATA_DIR)
    ...
    def tearDown(self):
        ...
        super(TestJavaMaven, self).tearDown()
    ...
    def test_build_single_build_with_deps_broken(self):
        source_dir = os.path.join(self.ro_source_dir, 'with-deps-broken')
