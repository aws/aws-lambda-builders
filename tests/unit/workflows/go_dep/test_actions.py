...
class TestGoBuildAction(TestCase):
    ...
    @patch("aws_lambda_builders.workflows.go_dep.subproc_exec.SubprocessExec")
    def test_runs_go_build_with_arm_architecture(self, SubProcMock):
        """
        tests the happy path of running `dep ensure`
        """

        sub_proc_go = SubProcMock.return_value
        action = GoBuildAction("base", "source", "output", sub_proc_go, ARM64, env={})

        action.execute()

        sub_proc_go.run.assert_called_with(
            ["build", "-o", "output", "source"], cwd="source", env={"GOOS": "linux", "GOARCH": "arm64"}
        )

    @patch("aws_lambda_builders.workflows.go_dep.subproc_exec.SubprocessExec")
    def test_fails_go_build_with_arm_architecture(self, SubProcMock):
        """
        tests failure, something being returned on stderr
        """
        ...
        sub_proc_go = SubProcMock.return_value
        sub_proc_go.run.side_effect = ExecutionError(message="boom!")
        action = GoBuildAction("base", "source", "output", sub_proc_go, "unknown_architecture", env={})
