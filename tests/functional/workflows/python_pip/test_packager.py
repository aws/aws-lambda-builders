...
@pytest.fixture
def sdist_reader():
    return SDistMetadataFetcher(sys.executable)
...
class PipSideEffect(object):
    ...
    def _build_fake_whl(self, directory, filename):
        ...
        if not os.path.isfile(filepath):
            package = Package(directory, filename, sys.executable)
...
class TestPackage(object):
    def test_same_pkg_sdist_and_wheel_collide(self, osutils, sdist_builder):
        with osutils.tempdir() as tempdir:
            ...
            pkgs = set()
            pkgs.add(Package("", "foobar-1.0-py3-none-any.whl", sys.executable))
            pkgs.add(Package(tempdir, "foobar-1.0.zip", sys.executable))
            assert len(pkgs) == 1
