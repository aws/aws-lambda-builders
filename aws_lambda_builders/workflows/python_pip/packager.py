...
class SDistMetadataFetcher(object):
    ...
    def __init__(self, osutils=None, python_exe=None):
        if osutils is None:
            osutils = OSUtils()
        self._osutils = osutils
...
class SubprocessPip(object):
    ...
    def __init__(self, osutils=None, import_string=None, python_exe=None):
        if osutils is None:
            osutils = OSUtils()
        self._osutils = osutils
        if import_string is None:
            import_string = pip_import_string(python_exe=python_exe)
...
class PipRunner(object):
    ...
    def _determine_abi(self):
        cmd = [
            self.python_exe,
            "-c",
            "import sys; assert sys.version_info.major == 3"
        ]
        p = self._osutils.popen(cmd, stdout=self._osutils.pipe, stderr=self._osutils.pipe)
        p.communicate()
        return 'cp36m' if p.returncode == 0 else 'cp27mu'
    ...
    def download_manylinux_wheels(self, packages, directory):
        ...
        for package in packages:
            arguments = ['--only-binary=:all:', '--no-deps', '--platform',
                         ...
                         '--abi', self._determine_abi(), '--dest', directory, package]
