...
class ExecutableNotFound(PackagerError):
    def __init__(self, bin_path):
        super(ExecutableNotFound, self).__init__(
            'Executable not found: %s' % bin_path)
...
class CargoParser(object):
    ...
    def get_executable_path(self, target_platform):
        ...
        if not os.path.isfile(bin_full_path):
            raise ExecutableNotFound(bin_full_path)
    ...
    def validate(self, print_warnings=True):
        ...
        if "package" not in self.cargo:
            raise CargoValidationError("Cargo file is not a valid package")
